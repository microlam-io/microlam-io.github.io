...
====
The last parameter in the `Aws` constructor is the `boolean` `useS3TransferAcceleration`.
It is so useful for reducing your deployment time that it is true by default but this can cause an increase in your AWS billing.
====
...
Microlam quickstart project is now including the class `[package].params.SystemParameters` you 

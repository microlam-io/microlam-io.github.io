...
* an IDE
* JDK 21+ installed with `JAVA_HOME` configured appropriately (The best is to use https://aws.amazon.com/corretto/[Amazon Corretto] build of OpenJDK)
...
The generated configuration is updated (and merged) every 30s in the folder: `target/[artifactId]-[version]-native-config/java17-amd64/function/config/`.
